...
$sql_count  = 'SELECT COUNT(*) FROM proyectos WHERE ' . implode(' AND ', $where);
...
function proyectoPagUrl(int $pag): string {
  $get = $_GET;
  unset($get['pagina']);
  ...
  <div style="max-width:1100px;margin:0 auto;padding:0 var(--space-md)">
    ...
    <?php else: ?>
      <div class="blog-grid">
        <?php foreach ($proyectos as $pro): ?>
          <a href="/proyectos/<?php echo urlencode($pro['slug']); ?>" class="blog-card">
            <?php if ($pro['imagen']): ?>
              <div class="blog-card-img"><img src="<?php echo htmlspecialchars(imgUrl($pro['imagen'], $base_url)); ?>" alt="<?php echo htmlspecialchars($pro['titulo']); ?>" loading="lazy"></div>
            <?php else: ?>
      ...
      <div class="paginacion">
        <p class="pag-info">
          Mostrando <?php echo $offset + 1; ?>–<?php echo min($offset + $por_pagina, $total_items); ?> de <?php echo $total_items; ?> proyectos
        </p>

        <?php if ($pagina_actual > 1): ?>
          <a href="<?php echo proyectoPagUrl($pagina_actual - 1); ?>" class="pag-btn">← Anterior</a>
        <?php else: ?>
        ...
        <?php for ($i = 1; $i <= $total_pags; $i++):
          if ($i === 1 || $i === $total_pags || abs($i - $pagina_actual) <= 2): ?>
            <a href="<?php echo proyectoPagUrl($i); ?>" class="pag-num <?php echo $i === $pagina_actual ? 'active' : ''; ?>"><?php echo $i; ?></a>
        ...
        <?php if ($pagina_actual < $total_pags): ?>
          <a href="<?php echo proyectoPagUrl($pagina_actual + 1); ?>" class="pag-btn">Siguiente →</a>
        <?php else: ?>
